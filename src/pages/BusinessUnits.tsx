        ...
        <div className="container">
            ...
            {t.businessUnits.items.map((unit, i) => (
              <ScrollReveal key={unit.name} delay={i * 90}>
                <div className="bg-background p-8 md:p-10 lg:p-12 h-full flex flex-col">
                  ...
                  <h3 className="text-foreground text-base md:text-lg font-light mb-4 tracking-[-0.01em]">{unit.name}</h3>
                  <p className="text-muted-foreground text-sm leading-[1.7] mb-8 flex-1">{unit.desc}</p>
                  {i === 0 ? (
                    <Link to={unit.href} className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.18em] text-foreground/60 hover:text-accent transition-colors duration-200">
          ...
          <ScrollReveal>
            <div className="mt-16 pt-10 border-t border-border/60 text-center">
              <p className="text-muted-foreground/50 text-[11px] tracking-[0.15em] uppercase">{t.footer.domizilNote}</p>
            </div>
          </ScrollReveal>
        </div>
      </section>
