            ...
            <div className="md:col-span-7">
              <ScrollReveal>
                <form className="space-y-10" onSubmit={(e) => e.preventDefault()}>
                  <div>
                    <label className="block text-muted-foreground/50 text-[11px] uppercase tracking-[0.18em] mb-3">{t.contact.formName}</label>
                    <input type="text" className="w-full bg-transparent border-b border-border py-3.5 text-foreground text-sm focus:outline-none focus:border-accent transition-colors duration-200 placeholder:text-muted-foreground/25" />
                  </div>
                  <div>
                    <label className="block text-muted-foreground/50 text-[11px] uppercase tracking-[0.18em] mb-3">{t.contact.formEmail}</label>
                  ...
                  <div>
                    <label className="block text-muted-foreground/50 text-[11px] uppercase tracking-[0.18em] mb-3">{t.contact.formSubject}</label>
                    <select
                      value={subject}
                ...
                <div className="md:pt-4">
                  <div className="mb-16">
                    ...
                    <a href={`mailto:${t.contact.email}`} className="flex items-center gap-3 text-foreground text-sm hover:text-accent transition-colors duration-200 mb-5">
                      <Mail size={14} strokeWidth={1.5} className="text-accent/40" />{t.contact.email}
                    </a>
                    <a href={`tel:${t.contact.phone.replace(/\s/g, "")}`} className="flex items-center gap-3 text-foreground text-sm hover:text-accent transition-colors duration-200">
                      <Phone size={14} strokeWidth={1.5} className="text-accent/40" />{t.contact.phone}
                    </a>
                  </div>
