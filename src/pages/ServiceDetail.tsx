    ...
    <>
      <section className="bg-soft-ivory pt-36 pb-24 md:pt-44 md:pb-32">
        <div className="container">
          <Link to="/leistungen" className="inline-flex items-center gap-2 text-warm-grey hover:text-[hsl(var(--gold-deep))] text-[11px] uppercase tracking-[0.18em] mb-10 transition-colors duration-200">
            <ArrowLeft size={12} />{t.servicePage.backToOverview}
          </Link>
          <p className="text-[hsl(var(--gold))] text-[11px] uppercase tracking-[0.2em] mb-5 animate-reveal-up" style={{ animationDelay: "100ms" }}>{service.number}</p>
      ...
      <section className="py-28 md:py-36 bg-background">
        <div className="container">
          <ScrollReveal>
            <p className="text-[hsl(var(--gold))] text-[11px] uppercase tracking-[0.2em] mb-5">{t.servicePage.whatWeSupport}</p>
          </ScrollReveal>
      ...
      <section className="py-24 md:py-32 bg-cloud-neutral">
        <div className="container">
          <ScrollReveal>
            <p className="text-[hsl(var(--gold))] text-[11px] uppercase tracking-[0.2em] mb-5">{t.servicePage.typicalSituations}</p>
          </ScrollReveal>
      ...
      {matchingIndustries.length > 0 && (
        <section className="py-24 md:py-32 bg-background">
          <div className="container">
            <ScrollReveal>
              <p className="text-[hsl(var(--gold))] text-[11px] uppercase tracking-[0.2em] mb-5">{t.servicePage.relevantIndustries}</p>
            </ScrollReveal>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
              {matchingIndustries.map((ind, i) => (
                <ScrollReveal key={ind.slug} delay={i * 80}>
                  <Link to={`/branchen/${ind.slug}`} className="block bg-card border border-border p-8 group hover:bg-soft-ivory transition-colors duration-300">
                    <h3 className="text-foreground text-base font-light mb-3 tracking-[-0.01em] group-hover:text-[hsl(var(--gold))] transition-colors duration-200">{ind.name}</h3>
