    ...
    <>
      <section className="relative bg-soft-ivory pt-36 pb-24 md:pt-44 md:pb-32 overflow-hidden">
        <div className="absolute inset-0">
          <img src={heroInsights} alt="Insights" className="w-full h-full object-cover opacity-[0.22]" loading="eager" />
            ...
            {t.insights.items.map((item, i) => (
              <ScrollReveal key={item.slug} delay={i * 80}>
                <article className="bg-background p-8 md:p-10 lg:p-14 h-full flex flex-col group">
                  ...
                  <p className="text-foreground/60 text-sm leading-[1.7] mb-8 flex-1">{item.excerpt}</p>
                  <span className="inline-flex items-center gap-2.5 text-[11px] uppercase tracking-[0.18em] text-foreground/25 group-hover:text-[hsl(var(--brand-blue))] transition-colors duration-200">
                    {t.insights.readMore}
