          ...
          {t.businessUnits.items.map((unit, i) => (
            <ScrollReveal key={unit.name} delay={i * 80}>
              <div className="bg-background p-7 md:p-9 h-full flex flex-col">
                ...
                <p className="text-muted-foreground text-sm leading-[1.7] mb-6 flex-1">
                  {unit.desc}
                </p>
                {i === 0 ? (
                  <Link
                    to={unit.href}
                    className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.18em] text-foreground/50 hover:text-accent transition-colors duration-200"
                  >
                    {t.domizil.cta}
